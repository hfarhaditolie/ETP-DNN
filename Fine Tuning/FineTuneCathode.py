...
class Conv1DInceptionModel(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(32000, 64)  # Fully connected layer
...
data= np.hstack((data[:,-5:-3],data[:,-2:-1],X)).astype(float)
# data= np.hstack((np.zeros((X.shape[0],3)),X)).astype(float)
...
for fold, (train_index, test_index) in enumerate(kf.split(data)):
    ...
    checkpoint = torch.load("checkpoints/cathode_epoch_45.pth")
    model.load_state_dict(checkpoint['model_state_dict'])
    ...
    with torch.no_grad():
        ...
        for inp, lbl in test_loader:
            ...
            y_true.append(lbl.cpu().numpy())   # Append all 16 values from lbl
            y_pred.append(prediction.cpu().numpy())  # Append all 16 predicted values
        ...
        y_pred = np.array(y_pred)
            ...
            # Calculate metrics
    rmse = np.sqrt(mean_squared_error(np.squeeze(y_true),  np.squeeze(y_pred)))
    mae = mean_absolute_error(np.squeeze(y_true),  np.squeeze(y_pred))
    r2 = r2_score(np.squeeze(y_true),  np.squeeze(y_pred))
