...
y_anode = data[:, 0]  # First column as target (Thickness)
# X_anode = data[:, 2004:] 
...
scaled_X = scaler.fit_transform(X_anode[:,2:])
data = np.hstack((X_anode[:,:2],X_anode[:,2:])).astype(float)
# data = scaled_X.astype(float)
targets = y.astype(float)
# data = data.reshape(-1, 1, 1002) 
# data = np.pad(data, ((0, 0), (0, 2000)), 'constant', constant_values=0)
# data = np.repeat(data, 2, axis=1)
targets = y_anode.astype(float)
data = data.reshape(-1, 1, 1002) 
...
fold_r_squared = []
fold_mape=[]
# synthetic_data = np.load("synthetic_feat.npy")  # First column as target (Thickness)
# synthetic_target = np.load("synthetic_targets.npy") 
...
for fold, (train_index, test_index) in enumerate(kf.split(data)):
    ...
    checkpoint = torch.load("checkpoints/anode_epoch_50.pth")
    model.load_state_dict(checkpoint['model_state_dict'])
    ...
    # X_train=np.concatenate((X_train, np.expand_dims(synthetic_data, axis=1)),axis=0)
    # y_train=np.concatenate((y_train,synthetic_target),axis=0)
    ...
    for epoch in range(num_epochs):
        for inputs, labels in train_loader:
            ...
            print(f'\rEpoch [{epoch + 1}/{num_epochs}], Loss: {loss.item():.4f}', end='', flush=True)

        
        # save_checkpoint(epoch, model, optimizer, loss, f'checkpoints/anode_epoch_{epoch+1}.pth')
        # Test the model
    rmse = []
    mae = []
    r_squared = []
    ...
    # print(f'\rMAE: {mae:.4f},RMSE: {rmse:.4f}, R²: {r2:.4f}', end='', flush=True)
    mape=mean_absolute_percentage_error(np.squeeze(y_true), np.squeeze(y_pred))
    fold_rmse.append(rmse)
    fold_mae.append(mae)
    fold_r_squared.append(r2)
    fold_mape.append(mape)

    print(f'Fold {fold + 1} RMSE: {fold_rmse[-1]:.4f}, MAE: {fold_mae[-1]:.4f}, MAPE: {fold_mape[-1]:.4f}, R²: {fold_r_squared[-1]:.4f}')
