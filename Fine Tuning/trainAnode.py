...
min_val, max_val = np.max(y) , np.min(y)
n_classes = 10
interval_size = (max_val - min_val) / n_classes
y_anode = []
X_anode = []
for i in range(10):
    feat = np.load("../Anode/Synthetic/backecho/synthetic_class_"+str(i)+".npy")
    tar = min_val + (np.ones(20)*i + 0.5) * interval_size
    y_anode.append(tar)
    X_anode.append(feat)
X_anode=np.concatenate(X_anode,axis=0)
y_anode=np.concatenate(y_anode,axis=0)
shuffle_indices = np.random.permutation(len(y_anode))

# Apply the permutation to shuffle X_anode and y_anode
X_anode = X_anode[shuffle_indices]
y_anode= y_anode[shuffle_indices]
from scipy.signal import find_peaks, medfilt
plt.plot(range(1000),medfilt(X_anode[0,:],31))
plt.show()
# Define target variable and features
# y_anode = data[:, 0]  # First column as target (Thickness)
# X_anode = data[:, 2004:] 

# y_anode =np.load("synthetic_targets.npy")  # First column as target (Thickness)
# X_anode = np.load("synthetic_feat.npy") 
import torch
import torch.nn as nn
...
# Prepare the dataset
# X_anode = np.hstack((np.zeros((X_anode.shape[0],2)),X_anode))
data = X_anode.astype(float)
targets = y_anode.astype(float)
data = data.reshape(-1, 1, 1000) 
